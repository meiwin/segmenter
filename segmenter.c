...
void display_usage(void)
{
    ...
    printf("\t-o, --output-folder FOLDER   Output folder\n");
    printf("\n");
    printf("\n");
...
int main(int argc, char **argv)
{
    ...
    static const char *optstring = "i:d:p:m:u:n:f:ovh?";
    ...
    output_filename = malloc(sizeof(char) * (strlen(options.output_prefix) + 200));
    if (!output_filename) {
    ...
    *dot = '.';
    ...
    snprintf(output_filename, strlen(options.output_folder) + 1 + strlen(options.output_prefix) + 15, "%s/%s-%u.ts", options.output_folder, options.output_prefix, output_index++);
